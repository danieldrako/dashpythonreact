...
const NivRepetidosComponent  = ({ data, repetidosData }) => {
  ...
  const options = {
    ...
    title: {
      text: 'Importe Total por Emisor en Repetidos'
    },
    xAxis: {
      categories: repetidosData.map(item => item.emisor),
      title: {
        text: 'Emisor'
      }
    },
    ...
    series: [{
      name: 'Importe Total',
      data: repetidosData.map(item => item.importe)
    }]
  };
  ...
  return (
    <div>
      <h2> Numero de identificacion vehicular repetidos </h2>
      <input 
        type="text"
        value={filtering}
...
export default NivRepetidosComponent